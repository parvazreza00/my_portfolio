...
class GeneralController extends Controller
{
    ...
    public function tech_web_banner_title_store(Request $request)
    {
        ...
        Image::make($image)->resize(1920, 920)->save('backend/banner_image/' . $image_gen);
    ...
    // website banner status active inactive method end ------------

    //cv all method start =======================================
    public function tec_web_cv_store(Request $request){

        if($request->hasFile('cv_file')){
            
            $file = $request->file('cv_file');
            $fileName = $file->getClientOriginalName();
            $file->move(public_path('backend/cv/'),$fileName);
            $url = 'backend/cv/'.$fileName;//save image in db;
            Cvfile::insert([
                'cv_file' => $url,            
                'created_at' => Carbon::now()
            ]);
            ...
            $notification = array(
                'message' => 'You Did not Select File!',
                'alert-type' => 'error'
            );
            return redirect()->back()->with($notification);
        }            
    ...
    // website bnner status active inactive method start ------------
    public function tec_web_cv_inactive($id)
    {
        Cvfile::findOrFail($id)->update(['status' => '0']);
        return redirect()->back();
    }
    public function tec_web_cv_active($id)
    {
        Cvfile::findOrFail($id)->update(['status' => '1']);
        return redirect()->back();
    }
    // website banner status active inactive method end ------------
    //cv all method end =======================================
